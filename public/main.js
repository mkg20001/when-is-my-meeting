...
/*
function hookField (id, notAllowedRe, handler) {
  id = `#${id}`

  function handle (e) {
    let val = $(id).val()
    val = val.replace(notAllowedRe, '')
    val = handler(val, e, e.originalEvent.key)

    $(id).val(val)
  }

  $(id).on('keydown', (e) => {
    if (e.originalEvent.key.length === 1 && !e.originalEvent.key.replace(notAllowedRe, '').length) {
      e.preventDefault()
    }
...
hookField('time', /[^0-9: apm]/gmi, (val, e, key) => {
  const validRe = /^[0-9]{1,2}:[0-9]{1,2} ?(am|pm)?$/gmi
